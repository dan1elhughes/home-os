...
The four database servers that used to run on the swarm, as one TrueNAS SCALE
...
hosts of `10.10.10.60:5432`. Three Postgres instances cannot share one port on
...
correct. `immich` and `gitea` compose files in this repo use 5433 and 5434.
Confirm this scheme at Phase 0 or replace it with per-container IPs.

## Deploy on TrueNAS

1. Create `/mnt/SSD/db/{immich,homeassistant,gitea,kuma}`.
...
4. Restrict inbound 5432-5434 and 3306 to the swarm subnet (10.10.10.21-23).

## Verify

```sh
nc -z 10.10.10.60 5432 && nc -z 10.10.10.60 5433 \
  && nc -z 10.10.10.60 5434 && nc -z 10.10.10.60 3306
```
