...
files/swarm-join.service.tmpl   all nodes: docker swarm join with baked token
files/swarm-init.service.tmpl   bootstrap only, unused by the migration
...
|---|---|
| `common` | `core` SSH keys, passwordless sudo, SSH hardening, hostname, update/reboot policy, host resolver pin (`10-cluster.network`) |
...
- **`KEEPALIVED_INTERFACE`** is `enp1s0`, taken from the existing nodes
  ...
  feeds `10-cluster.network`.
- **Resolvers are pinned to the dns-stack VIP (`10.10.10.20`).**
  `10-cluster.network` is a full networkd unit (`DHCP=yes`, `DNS=10.10.10.20`,
  `[DHCPv4] UseDNS=false`); networkd honours the first lexically-matching unit,
  so `10-` beats anything the platform ships and the unit must carry the DHCP
  config itself. `/etc/docker/daemon.json` pins containers — dockerd never
  re-reads host resolver changes. Without both, a node and its containers fall
  back to router DNS that cannot resolve `*.danhughes.dev`. Both name the
  keepalived VIP; change them together if it ever moves.
...
- **No node runs `swarm init`.** All three join the live swarm with the baked
  token; the migration never re-initialises (a cl01 init would fork a second
  cluster while the old nodes still hold the real one).
  `swarm-init.service.tmpl` is kept only to bootstrap a from-scratch rebuild.
